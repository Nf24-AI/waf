 ...
 */
function DisplayMode({ meeting, onBack, onRefresh, copy, language }: { meeting: MeetingRecord; onBack: () => void; onRefresh: () => void; copy: { back: string; refresh: string; fullScreen: string; exitFullScreen: string; goal: string; topics: string; actions: string; notes: string; attendees: string }; language: "ar" | "en" }) {
  ...
  return (
    ...
    >
      <header className="display-toolbar">
        ...
        <div className="display-brand">واف<span className="display-divider" /><span style={{ font: "var(--text-caption)", color: "var(--text-tertiary)" }}>{isArabic ? "صفحة الاجتماع" : "Meeting page"}</span></div>
            ...
            : (
              <div className="stage-agenda">
                {meeting.agenda.map((item, index) => (
                  <article className="stage-point" key={`${item.title}-${index}`}>
